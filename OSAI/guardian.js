...
class OSAI {
  ...
  init() {
    ...
    window.fetch = async (...args) => {
      ...
      if(typeof url === 'string' && this.looksMalicious(url)){
        ...
        if(choice === 0) throw new Error('Blocked by OSAIGuardian')
      }
  ...
  toggle(on){
    ...
    nexusSpeak(on ? "OSAIGuardian ativado. Estou de olho em tudo" : "OSAIGuardian desligado. Você manda 100% agora")
  }
}

window.guardian = new OSAIGuardian()
